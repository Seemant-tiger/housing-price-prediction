...
from yellowbrick.target import BalancedBinningReference, ClassBalance
...
class Normality:
    ...
    def fit(self, X, y=None):
        ...
        self.X = X
        self.y = y
        self.visualizer = Rank1D(algorithm="shapiro", features=self.X.columns)
        self.visualizer.fit(self.X, self.y)
        self.ranks_ = self.rank(self.X)
        # self.visualizer.transform(self.X)
        return self
    ...
    def get_plot_data(self):
        ...
        ranks = pd.Series(self.ranks_).sort_index(ascending=False)
        features = pd.Series(self.visualizer.features_)
        normality_ranks = pd.DataFrame(
            {"features": features, "ranks": ranks}
        ).set_index(["features"])
...
class TargetDistribution:
    ...
    def __init__(self, is_classification):
        ...
        if self.is_classification:
            self.model = ClassBalance()
        else:
            self.model = BalancedBinningReference()
