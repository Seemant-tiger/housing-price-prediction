...
import pandas as pd
from yellowbrick.features import Manifold, PCADecomposition

from .scoring import FeatureCorrelation


class PCAProjection(PCADecomposition):
    """
    ...
    def fit_transform(self, X, y=None, **kwargs):
        ...
        """
        self.X = X
        self.dims = pd.DataFrame(super().fit_transform(X, y))
        ...
        if hasattr(super(), "ax"):
            plt.delaxes(super().ax)
        if hasattr(super(), "cax"):
            plt.delaxes(super().cax)
        return self.dims
